...
describe('DbAddSurvey Usecase', () => {
  ...
  it('should throw if SaveSurveyResultRepository throws', async () => {
    const { sut, saveSurveyResultRepositoryStub } = makeSut()
    jest.spyOn(saveSurveyResultRepositoryStub, 'save').mockImplementationOnce(async () => await new Promise((resolve, reject) => reject(new Error())))
  ...
  it('should return a list of SurveyResult on success', async () => {
    const { sut } = makeSut()
    const saveResult = await sut.save(makeFakeSurveyResultData())
